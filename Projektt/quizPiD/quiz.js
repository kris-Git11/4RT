...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var teams = ['TOCNO', 'NETOCNO'];

  var strPomoc = 'Pomoc';
  var strZatvori = 'Zatvori';
  ...
  (function() {
    ...
    tieLink   = addNewElement(addNewElement(linksList, 'li'), 'button', strPomoc);
    ...
    closeLink = addNewElement(addNewElement(el, 'li'), 'button', strZatvori);
    ...
    for (var i = 1; i <= max; ++i) {
      ...
      categories.forEach(function(cat) {
        ...
        if (i <= cat.questions.length) {
          ...
          cell.onclick = makeClickHandler(cat.questions[i - 1], i);
        } else {
          addText(cell, '\u00A0');
        }
      });
